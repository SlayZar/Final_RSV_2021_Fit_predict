...
def interpret(df, model_number):
 ...
 #   MODEL_COLS = joblib.load(f'models/cols_{model_number}') 
    ...
    pl.clf()
    ntree=st.number_input('Выберите строку, для которой хотим вывести интерпретацию модели'
                                      , min_value=0
                                      , max_value=len(df)
                                      )
    ...
    pl.clf()
    if st.button('Значения SHAP values для данного наблюдения'):
...
def eda_func(df):
    ...
    options2 = st.selectbox('Что хотим сделать?',
             ['Посмотреть зависимости (EDA)','Интерпретация модели'], index=0)
    if options2 == 'Посмотреть зависимости (EDA)':
        ...
        model_number = st.selectbox('Выберите модель',
             ['Сегмент 1 (25-31 Ж)', 'Сегмент 2 (25-42 М - Пиво)', 'Сегмент 3 (25-43 Ж - Дети)', 
              'Сегмент 4 (18-44 М и Ж - Животные)', 'Сегмент 5 (18-45 М и Ж)'], index=0)
...
if options == 'Семпл данных':
    df = pd.read_csv('data/data_sample_small.csv')
