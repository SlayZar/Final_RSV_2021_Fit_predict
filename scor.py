...
#     non_numeric_columns.append(None)
    ...
    if options2 == 'Посмотреть зависимости (EDA)':
        ...
        model_number = st.selectbox('Выберите модель',
             ['Сегмент 1 (25-31 Ж)', 'Сегмент 2 (25-42 М - Пиво)', 'Сегмент 3 (25-43 Ж - Дети)', 
              'Сегмент 4 (18-44 МЖ - Животные)', 'Сегмент 5 (18-45 М и Ж)'], index=0)
...
def get_df_to_cluster(pred_df_train):
  to_cluster = pred_df_train[['gamecategory', 'subgamecategory', 'timezone',
                            'phone_ver']]
  to_cluster['game_type'] = to_cluster.apply(lambda x: x['gamecategory'] + '_' +
                  x['subgamecategory'], axis=1)
  for i in ['phone_ver', 'game_type']:
    to_cluster = to_cluster.merge(pd.get_dummies(to_cluster[i]), left_index=True,
                                  right_index=True)
    to_cluster.drop([i], axis=1, inplace=True)
  to_cluster.drop(['gamecategory', 'subgamecategory'], axis=1, inplace=True)
  return to_cluster
        
def hierarch(customers):
    df = get_df_to_cluster(customers)
